...
deploy: patch-il check-shaders ## Deploy Cecil-patched DLLs into vanilla client dir and install dir
	...
	@find sources/shaders -maxdepth 1 -type f -exec cp -f {} $(VANILLA_DIR)/assets/game/shaders/ \;
	...
	@if [ -d "sources/shaderincludes" ]; then mkdir -p $(VANILLA_DIR)/assets/game/shaderincludes; cp sources/shaderincludes/* $(VANILLA_DIR)/assets/game/shaderincludes/; fi
	...
	@for f in sources/shaders/* sources/shaderincludes/*; do [ -f "$$f" ] || continue; d="$(VANILLA_DIR)/assets/game/$$(echo $$f | cut -d/ -f2)/$$(basename $$f)"; [ -f "$$d" ] || { echo "Error: $$f did not reach $$d"; exit 1; }; done
	...
	@if [ -d "$(INSTALL_DIR)" ]; then \
		...
		find sources/shaders -maxdepth 1 -type f -exec cp -f {} $(INSTALL_DIR)/assets/game/shaders/ \;; \
		if [ -d "sources/shaderincludes" ]; then mkdir -p $(INSTALL_DIR)/assets/game/shaderincludes; cp sources/shaderincludes/* $(INSTALL_DIR)/assets/game/shaderincludes/; fi; \
		for f in sources/shaders/* sources/shaderincludes/*; do [ -f "$$f" ] || continue; d="$(INSTALL_DIR)/assets/game/$$(echo $$f | cut -d/ -f2)/$$(basename $$f)"; [ -f "$$d" ] || { echo "Error: $$f did not reach $$d"; exit 1; }; done; \
