...
3b. Steering a running session: `codex queue --thread <id> --message "<user's words>"` reaches it
   only while it runs; a message queued after exit is lost. To continue an exited session:
   `codex exec resume <session-id> "<next brief>"` through the same wrapper. The monitor pattern must
   be `^CODEX_EXIT [0-9]+$`; Codex prints the word CODEX_EXIT in its own narration and false-matched
   a looser pattern.
3c. Relay user observations verbatim and in time ("still jittering, no AA", "gets worse with
   distance") - each one narrowed the search; do not translate them into your own hypothesis.
