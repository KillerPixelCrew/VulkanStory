...
internal sealed class DlssUpscaler : IDisposable
{
    ...
    public static DlssUpscaler? TryPrepare(string applicationDataPath, Action<string>? log = null,
        bool prepareForSwitching = false)
    {
        var host = new DlssUpscaler(log);
        if (!Requested && !prepareForSwitching)
        {
        ...
        {
            if (Requested) host.Fail("NGX was already shut down in this process and allows exactly one lifetime");
            return null;
        }
        ...
        if (diagnosis != null)
        {
            if (Requested) host.Fail(diagnosis);
            return null;
        }
    ...
    {
        Unavailable = reason;
        bool first = Requested && OptimumConfig.DisableUpscalerAtRuntime();
        if (first)
        {
