...
public partial class VulkanClientPlatform
{
    ...
    /// <c>Screenshot.GrabScreenshot</c> - the screenshot key and the AVI recorder - decodes
    /// into an <c>SKBitmap</c> declared <c>Bgra8888</c>, and the headless harness writes
    /// its frames from the same call. The device's default colour target is
    /// <c>R8G8B8A8_UNORM</c>, so without this every Vulkan screenshot and recording comes
    ...
    /// left at the base's true either way, because this method makes it true.</para>
    /// </summary>
