...
///
/// Where those libraries are looked for is the last section of this file, and it is a
/// pure path question - no driver, no device, no NGX. They are NVIDIA
/// redistributables, so they are never in this repository and never in the game's
/// assets: they ship beside the client, next to <c>Optimum.Render.Vulkan.dll</c> and
/// the Silk.NET natives, or in a <c>dlss</c> folder there. A deployed client has no
/// <c>OPTIMUM_NGX_FEATURE_PATH</c> set and no repository checkout above it, so the
/// application directory has to be one of the searched candidates or the upscaler is
/// unreachable outside a development tree - which is how it failed before
/// 2026-09-12.
/// </summary>
...
public class NgxAvailabilityTests
{
    ...
    // ---- where the feature libraries are looked for -------------------------
