...
/// <summary>
/// Can Optimum drive DLSS Super Resolution through NGX on native Vulkan?
///
...
public class NgxAvailabilityTests
{
    ...
    public void TheShimIsTheBuildsOwnAndReportsItsAbiVersion()
    {
        ...
        Assert.Equal(OperatingSystem.IsWindows() ? "nvngx.dll" : NgxInterop.LibraryName,
            NgxShim.RuntimeName);
    }
    ...
    /// SuperSampling.Available = 1 and valid DLSS optimal settings at 2560x1490.
    ///
    /// The supported dimensions are asserted, not just reported: they show that
    ...
    public void NgxComesUpFromManagedCodeThroughTheShimAndReportsWhatTheNativeProbeSaw()
    {
        ...
        try
        {
            ...
            Assert.InRange(qualitySettings.OptimalWidth, 1u, 2560u);
            Assert.InRange(qualitySettings.OptimalHeight, 1u, 1490u);
            ...
            Assert.InRange(perfSettings.OptimalWidth, 1u, 2560u);
            Assert.InRange(perfSettings.OptimalHeight, 1u, 1490u);
        }
        finally
        {
