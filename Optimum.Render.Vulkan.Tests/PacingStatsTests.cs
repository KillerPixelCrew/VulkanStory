...
public class PacingStatsTests
{
    ...
    public void NewStatsLinesCarryStableKeyValueTokens()
    {
        ...
        Assert.Equal(
            ...
            "native_passes=25 native_draws=26 native_fullscreen_draws=27 native_mesh_draws=28 " +
            "native_instanced_draws=29 native_indirect_draws=30",
    ...
    public void SampleIsTheOriginalLineFollowedByTheTokenLines()
    {
        ...
        Assert.StartsWith("stats.shaders native=", lines[8]);
        // Caching follow-ups: pipelines compiled blocking/async/prewarmed, skipped draws, cache bytes, saves.
        Assert.StartsWith("stats.pipelines compiled_sync=", lines[7]);
