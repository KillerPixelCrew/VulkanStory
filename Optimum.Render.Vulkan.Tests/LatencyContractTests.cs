...
/// tracker, the None backend's behaviour, and the report buffer they all keep.
///
/// The report buffer is the one with a measured cost behind it (latency review,
/// 2026-09-12): the backends used a <c>List</c> with <c>RemoveAt(0)</c>, so once the
/// buffer was full - which it is for the whole session whenever nothing drains it,
/// and nothing does unless OPTIMUM_VULKAN_STATS is set - every present shifted the
/// entire array down by one inside the present path. The ring must drop the oldest
/// entry without moving anything and must keep the order the interface promises
/// (oldest first).
/// </summary>
public class LatencyContractTests
{
    ...
    // ---- the report buffer every CPU-timestamp backend keeps ---------------
