...
internal enum PatchArtifact { Donor, Vanilla }

/// <summary>One declared change to a vanilla assembly, with its donor source if any.</summary>
internal readonly record struct PatchOperation(
    PatchOperationKind Kind,
    string TypeName,
    string? MemberName,
    int? ParameterCount,
    IReadOnlyList<string>? ParameterTypes,
    PatchArtifact? Source,
    PatchArtifact Destination,
    bool Required);

/// <summary>
/// The donor-to-vanilla patch selection. Keep each target's name and overload
/// constraints here; the CLI only chooses inputs and runs this manifest.
/// </summary>
internal sealed record PatchManifest(
    ...
    Dictionary<string, List<string>> FieldsToRetype,
    List<string> TypesToUnseal,
    List<MethodTarget> MethodsToVirtualize,
    List<HookTarget> Hooks)
{
    public IEnumerable<PatchOperation> Operations()
    {
        foreach (string type in TypesToInject)
            yield return new(PatchOperationKind.InjectType, type, null, null, null,
                PatchArtifact.Donor, PatchArtifact.Vanilla, true);
        foreach ((string type, List<string> members) in MembersToInject)
            foreach (string member in members)
                yield return new(PatchOperationKind.InjectMember, type, member, null, null,
                    PatchArtifact.Donor, PatchArtifact.Vanilla, true);
        foreach ((string type, List<string> fields) in FieldsToRetype)
            foreach (string field in fields)
                yield return new(PatchOperationKind.RetypeField, type, field, null, null,
                    PatchArtifact.Donor, PatchArtifact.Vanilla, true);
        foreach (MethodTarget target in Targets)
            yield return new(PatchOperationKind.TransplantBody, target.TypeFullName,
                target.MethodName, target.ParamCount, target.ParameterTypes,
                PatchArtifact.Donor, PatchArtifact.Vanilla, !target.Optional);
        foreach (HookTarget hook in Hooks)
            yield return new(PatchOperationKind.HookCall, hook.TypeFullName,
                hook.MethodName, hook.ParamCount, null,
                PatchArtifact.Donor, PatchArtifact.Vanilla, !hook.Optional);
        foreach (string type in TypesToUnseal)
            yield return new(PatchOperationKind.UnsealType, type, null, null, null,
                null, PatchArtifact.Vanilla, true);
        foreach (MethodTarget target in MethodsToVirtualize)
            yield return new(PatchOperationKind.VirtualizeMethod, target.TypeFullName,
                target.MethodName, target.ParamCount, target.ParameterTypes,
                null, PatchArtifact.Vanilla, !target.Optional);
    }

    /// <summary>Existing source patches whose declaring types are changed by this manifest.</summary>
    public string[] CecilOwnedPatchPaths(string repositoryRoot) => Operations()
        .Select(operation => PatchPath(operation.TypeName))
        .Where(path => path != null && File.Exists(Path.Combine(repositoryRoot, path)))
        .Select(path => path!)
        .Distinct(StringComparer.Ordinal)
        .OrderBy(path => path, StringComparer.Ordinal)
        .ToArray();

    private static string? PatchPath(string typeName)
    {
        if (!typeName.StartsWith("Vintagestory.", StringComparison.Ordinal)) return null;
        int separator = typeName.LastIndexOf('.');
        if (separator < 0) return null;
        return "patches/VintagestoryLib/" + typeName[..separator] + "/" +
            typeName[(separator + 1)..] + ".cs.patch";
    }

    public static PatchManifest Create()
    {
        ...
        return new PatchManifest(typesToInject, membersToInject, targets,
            fieldsToRetype, typesToUnseal, methodsToVirtualize, hooks);
    }
}
