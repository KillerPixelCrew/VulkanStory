...
// Native port of taa-debug.vsh (docs/vulkan.md).
...
#extension GL_EXT_scalar_block_layout : require
#extension GL_GOOGLE_include_directive : require
// Native port of taa-debug.fsh (docs/vulkan.md).
//
...
// Program interface of taa-debug (docs/vulkan.md section 4). A fullscreen pass: one draw per
...
void main(void)
{
	...
	vec4 motion = texelFetch(optimumTextures2D[motionTex], pixel, 0);
	float sceneDepth = texelFetch(optimumTextures2D[depthTex], pixel, 0).r;
	...
	else if (mode == 4)
	{
		...
		vec3 scene = texture(optimumTextures2D[sceneTex], texCoord).rgb;
		vec2 mapped = clamp(motion.rg / 16.0, vec2(-1.0), vec2(1.0)) * 0.5 + 0.5;
	...
	{
		outColor = texture(optimumTextures2D[sceneTex], texCoord);
	}
}
